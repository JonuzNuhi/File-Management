...
function (ko, ojbootstrap_1, data, ArrayDataProvider){
    function ListViewModel() {
      ...
      this.dataProvider1 = new ArrayDataProvider(JSON.parse(data), { keyAttributes: "name", });

      //table chart
      this.serviceArray = ko.observableArray([]);
     
      callGetService('list/drive')
      .then((response) =>{
          ...
          if (row.values().size > 0) {
              row.values().forEach(function (key) {
                  ...
                  if (key) {
                    ...
                    var url = "http://localhost:8080/api/download/drive/" + key;
